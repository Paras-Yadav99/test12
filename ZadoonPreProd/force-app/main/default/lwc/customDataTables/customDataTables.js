...
import customCss from '@salesforce/resourceUrl/customDataTableCss';

const DELAY = 300;
const SHOWIT = 'visibility:visible';
const HIDEIT = 'visibility:hidden';
export default class CustomDataTables extends LightningElement {
    @api columns;
    @api
    set records(value){
    ...
    connectedCallback(){
        ...
        this.setupTable();
        console.log('lstData on Child::'+JSON.stringify(this._records));
        this.shownRecords= this.records.length;
    }
    ...
    renderedCallback(){
        ...
        let table = this.template.querySelector('.css-data-table');
    ...
    setupColumns(){
        ...
        this.columns.forEach(val => {
            ...
            col.thClass += val.resizable ? ' css-is-resizable' : '';
            ...
            if(col.sortable === true){
                col.thClass += ' css-is-sortable';
                col.sortBy = col.sortBy ? col.sortBy : col.fieldName;
                col.sortByTitle = 'Sort By: '+col.label;
                let sortIconClass = this.sortedDirection === 'asc' ? 'css-sort_desc' : 'css-sort_asc';
                col.sortIconStyle = 'visibility:hidden;';
                if(col.sortBy === this.sortedBy){
                    sortIconClass = this.sortedDirection === 'asc' ? 'css-sort_asc' : 'css-sort_desc';
                    col.sortIconStyle = 'visibility:visible;';
                }
                col.sortIconClass = 'css-sort-icon-div '+sortIconClass;
            }
            tempCols.push(col);
        });
    ...
    setupData(){
        ...
        this._records.forEach(value => {
            ...
            this.columns.forEach(col => {
                ...
                field.tdClass = col.editable ? 'css-cell-edit' : '';
                field.index = j++;
                field.mode = 'view';
                fields.push(field);
            });
        ...
        //this.tableData = recs;
         this.tableData =JSON.parse(JSON.stringify(recs));
    ...
    setupPageData(recs){
        ...
        recs.forEach(rec=>{
            let row = {...rec};
            row.rowClass = row.isSelected ? 'css-is-selected' : '';
    ...
    handleCellEdit(event){
        ...
        //this.tableData2Display[resp.rowIndex].mode = 'view';
        this.tableData2Display[resp.rowIndex].fields[resp.colIndex].mode = 'edit';
        this.setupPages();
    }
    handleCellChange(event){
        ...
        if(resp.value !== this._records[resp.rowIndex][resp.name]){
            this.tableData2Display[resp.rowIndex].fields[resp.colIndex].tdClass = 'css-cell-edit css-cell-edited';
            this.tableData2Display[resp.rowIndex].fields[resp.colIndex].value = resp.value;
            rec[resp.name] = resp.value;
        }else{
            this.tableData2Display[resp.rowIndex].fields[resp.colIndex].tdClass = 'css-cell-edit';
            this.tableData2Display[resp.rowIndex].fields[resp.colIndex].value = resp.value;
        }
        let changedFields = this.tableData2Display[resp.rowIndex].fields.filter(field=>field.tdClass.includes('css-cell-edited'));
    ...
    refreshEdit(){
    const toggleList = this.template.querySelectorAll('c-form-element');
    for (const toggleElement of toggleList) {
        toggleElement.closeEditMode();
    }
  }
